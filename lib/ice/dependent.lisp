...
;;; $Id: dependent.lisp,v 1.5 2005/09/18 17:48:15 ihatchondo Exp $
;;; ---------------------------------------------------------------------------
;;;     Title: ICE Library
...
(defun default-input-wait (input-stream timeout)
  "Sleeps until there is input on the STREAM, or for TIMEOUT seconds,
...
(defun connect-to-local-peer (host path &key (kind :stream) (bin t))
  "Connects to a local unix socket designed by the pathname and returns a 
...
(defun connect-to-tcp-peer (host port &optional (bin t))
  "Connects to a tcp socket to host at port, and returns a two way stream
   associated with."
  ...
  #+clisp (#+lisp=cl ext:socket-connect #-lisp=cl lisp:socket-connect port host
		     :element-type (if bin '(unsigned-byte 8) 'character))
  #+(or cmu scl)
  (sys:make-fd-stream (ext:connect-to-inet-socket host port)
       :buffering (if bin :full :line) :input t :output t
       :element-type (if bin '(unsigned-byte 8) 'character))
...
(defun connect-to-decnet-peer (host obj)
  "Connects to a decnet socket to host at/on obj, and returns a two way stream
   associated with."
